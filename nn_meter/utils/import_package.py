...
def try_import_onnx(require_version=["1.12.0", "1.10.0", "1.9.0"]):
    ...
    try:
        ...
        if version.parse(onnx.__version__).release[:2] not in [version.parse(v).release[:2] for v in require_version]:
            logging.warning(
                f'onnx=={onnx.__version__} is not well tested now, well tested version: onnx=={", ".join(require_version)}')
        return onnx
    except ImportError:
        logging.error(
            f'You have not install the onnx package, please install onnx=={require_version[0]} and try again.')
...
def try_import_torch(require_version=["1.10.0", "1.9.0", "1.7.1"]):
    ...
    try:
        ...
        if version.parse(torch.__version__).release[:2] not in [version.parse(v).release[:2] for v in require_version]:
            logging.warning(
                f'torch=={torch.__version__} is not well tested now, well tested version: torch=={", ".join(require_version)}')
        return torch
    except ImportError:
        logging.error(
            f'You have not install the torch package, please install torch=={require_version[0]} and try again.')
...
def try_import_tensorflow(require_version=["2.7.0", "2.6.0"]):
    ...
    try:
        import tensorflow
        if version.parse(tensorflow.__version__).release[:2] not in [
                version.parse(v).release[:2] for v in require_version]:
            logging.warning(
                f'tensorflow=={tensorflow.__version__} is not well tested now, well tested version: tensorflow=={", ".join(require_version)}')
        return tensorflow
    except ImportError:
        logging.error(
            f'You have not install the tensorflow package, please install tensorflow=={require_version[0]} and try again.')
...
def try_import_nni(require_version=["2.7", "2.6", "2.5", "2.4"]):
    ...
    try:
        ...
        if version.parse(nni.__version__).release not in [version.parse(v).release for v in require_version]:
            logging.warning(
                f'nni=={nni.__version__} is not well tested now, well tested version: nni=={", ".join(require_version)}')
        return nni
    except ImportError:
