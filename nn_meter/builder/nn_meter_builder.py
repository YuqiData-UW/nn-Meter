    ...
    for module in models.values():
        for id, model in module.items():
            ...
            if count % 50 == 0:
                ...
                logging.keyinfo(
                    f"{count} models complete. Still converting... Save the intermediate results to {info_save_path} ")
        ...
        for id, model in module.items():
            ...
            if have_converted:  # the models have been converted for the backend
                ...
                try:
                    model_path = model['model']
                    signal.alarm(time_threshold)
                    profiled_res = backend.profile_model_file(
                        model_path, model_save_path, input_shape=model['shapes'], metrics=metrics, **kwargs)
                    signal.alarm(0)
                    for metric in metrics:
                        model[metric] = profiled_res[metric]
                    time.sleep(0.2)
            ...
            if count > 0 and count % log_frequency == 0:
                ...
                logging.keyinfo(
                    f"{count} models complete. Still profiling... Save the intermediate results to {info_save_path} ")
...
def sample_and_profile_kernel_data(
        kernel_type,
        sample_num,
        backend,
        sampling_mode='prior',
...
def build_predictor_for_kernel(kernel_type, backend, init_sample_num=1000, finegrained_sample_num=10,
    ...
    predictor, acc10, error_configs = build_predictor_by_data(
        kernel_type, kernel_data, backend, error_threshold=error_threshold, mark=f'prior{mark}', save_path=os.path.join(
            workspace_path, "results"), predict_label=predict_label)
    ...
    for i in range(1, iteration):
        ...
        predictor, acc10, error_configs = build_predictor_by_data(
            kernel_type, kernel_data, backend, error_threshold=error_threshold, mark=f'finegrained{i}{mark}', save_path=os.path.join(
                workspace_path, "results"), predict_label=predict_label)
...
def build_initial_predictor_by_data(
        kernel_type,
        backend=None,
        init_sample_num=20,
        error_threshold=0.1,
    ...
    return build_predictor_for_kernel(
        kernel_type,
        backend,
...
def build_adaptive_predictor_by_data(
        kernel_type,
        kernel_data,
        backend=None,
    ...
    _, _, error_configs = build_predictor_by_data(
        kernel_type, kernel_data, backend=backend, error_threshold=error_threshold, save_path=None, predict_label=predict_label)
