...
def merge_info(new_info, info_save_path=None, prev_info=None):
    ...
    '''
    if (info_save_path is None and prev_info is None) or (info_save_path is not None and prev_info is not None):
        raise ValueError("One and only one params of `info_save_path` and `prev_info` is needed.")

    if info_save_path is not None and os.path.isfile(info_save_path):
...
def handle_timeout(sig, frame):
    raise TimeoutError(
        'Model profiling took too long (longer than the time threshold in the funciton `nn_meter.builder.profile_models`, default to be 300s)')
