        ...
        pkg_resources.resource_filename(
            ".".join(
                __name__.split('.')[
                    :
                    - 2]),
            f'configs/builder/fusion_rule_tester/'
            + __ruletest_cfg_filename__),
        os.path.join(
            os.path.join(
                workspace_path,
                'configs'),
            'ruletest_config.yaml'))
        ...
        pkg_resources.resource_filename(
            ".".join(
                __name__.split('.')[
                    :
                    - 2]),
            f'configs/builder/predictor_builder/'
            + __predbuild_cfg_filename__),
        os.path.join(
            os.path.join(
                workspace_path,
                'configs'),
            'predictorbuild_config.yaml'))
...
def load_config_file(workspace_path):
    ...
    try:
        ...
        except BaseException:  # In customized backend, sometimes there is no backend config file, skip loading config for this case
            backend = None
        ...
        raise FileNotFoundError(
            f"config file in {workspace_path} not found, please create the workspace first."
...
class ConfigData:
    ...
    def get_module(self, module=''):
        try:
            ...
            raise ValueError(
                f"Module [{module}] from config file not found, please create the workspace first. "
                "Docs: https://github.com/microsoft/nn-Meter/blob/main/docs/builder/overview.md#-create-workspace-")
