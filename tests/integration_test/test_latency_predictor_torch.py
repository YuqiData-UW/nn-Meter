...
def integration_test_onnx_based_torch(
        model_type,
        model_list,
        output_name="tests/integration_test/test_result_onnx_based_torch.txt"):
    """
    ...
    for pred_name, pred_version in get_predictors():
        try:
            since = time.time()
            print(" ".join(['nn-meter', 'lat_pred', '--torchvision'] + model_list
                  + ['--predictor', pred_name, '--predictor-version', pred_version]))
            result = subprocess.run(['nn-meter',
                                     'lat_pred',
                                     '--torchvision'] + model_list + ['--predictor',
                                                                      pred_name,
                                                                      '--predictor-version',
                                                                      pred_version],
            ...
            logging.error(
                f"Meets ERROR when checking --torchvision {model_list} --predictor {pred_name} --predictor-version {pred_version}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--apply-onnx",
        help='apply onnx-based torch converter for torch model',
        action='store_true',
        default=False)
    parser.add_argument(
        "--apply-nni",
        help='apply nni-based torch converter for torch model',
        action='store_true',
        default=False)
